...
function Profile() {
  ...
  const { data: profile } = useQuery({
    ...
    enabled: !!user,
    queryFn: async () => await getProfileFromFirestore(user!.id),
  });
  ...
  const Row = ({ to, icon, label }: { to: any; icon: React.ReactNode; label: string }) => (
    <Link
      to={to}
      className="flex items-center gap-3 rounded-2xl bg-surface p-4 ring-1 ring-border hover:ring-primary/40 transition-colors"
    >
  ...
  return (
    <div className="min-h-screen bg-background pb-28">
      ...
      <main className="mx-auto max-w-[430px] space-y-4 px-4 pt-4">
        <section className="rounded-3xl bg-gradient-to-br from-primary to-accent p-5 text-white shadow-md">
          <div className="flex items-center gap-4">
            <div className="size-16 overflow-hidden rounded-full bg-white/20 ring-2 ring-white/40 flex items-center justify-center font-bold text-xl">
              {profile?.avatar_url ? (
        ...
        <div className="space-y-2">
          ...
          <Row to="/terms" icon={<FileText className="size-4" />} label="Terms & Conditions" />
          <Row to="/privacy" icon={<Lock className="size-4" />} label="Privacy Policy" />
          {isAdmin && (
            <Row to="/admin" icon={<ShieldCheck className="size-4" />} label={t("admin")} />
          )}
        </div>

        <button
          onClick={signOut}
          className="flex w-full items-center justify-center gap-2 rounded-2xl bg-surface p-4 text-sm font-semibold text-destructive ring-1 ring-border"
        >
