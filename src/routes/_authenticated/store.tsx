...
import {
  getActiveSubscriptionPlansFromFirestore,
...
function Store() {
  ...
  const paymentConnected = isPaymentGatewayConnected();

  const { data: plans = [] } = useQuery({
    queryKey: ["plans-active"],
    queryFn: async () => await getActiveSubscriptionPlansFromFirestore(),
  });

  const visiblePlans = paymentConnected ? plans : plans.filter((p: any) => p.price === 0);
  ...
  async function buyPlan(plan: any) {
    ...
    try {
      ...
      await updateProfileInFirestore(user.id, patch);
    ...
    <div className="min-h-screen bg-background pb-28">
      ...
      <main className="mx-auto max-w-[430px] space-y-6 px-4 pt-4">
        ...
        {!paymentConnected && (
          <div className="rounded-2xl border border-amber-500/30 bg-amber-500/10 p-4 text-xs space-y-1.5">
            <div className="flex items-center gap-2 font-semibold text-amber-800 dark:text-amber-300">
              ...
              Payments Currently Offline
            </div>
            <p className="text-amber-900/80 dark:text-amber-200/90 leading-relaxed">
              Paid plan packages and promotions are temporarily hidden until a live payment gateway
              (Razorpay / Stripe / PhonePe) is connected. Enjoy 100% free posting across
              Lakshadweep!
            </p>
          </div>
        )}
            ...
            return (
              <div
                ...
                className={
                  ...
                  (highlight
                    ? "bg-gradient-to-br from-primary/10 to-accent/10 ring-primary/30"
                ...
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 text-2xl font-bold text-primary">
                      {p.price > 0 ? formatINR(p.price) : "Free"}
                  ...
                  {p.price > 0 && (
                    <Button size="sm" onClick={() => buyPlan(p)}>
                      Buy
                    </Button>
                  )}
                </div>
                <ul className="mt-3 space-y-1.5">
                  {(p.benefits ?? p.features ?? []).map((b: string, i: number) => (
                    <li key={i} className="flex items-start gap-2 text-xs">
                      <Check className="mt-0.5 size-3.5 shrink-0 text-accent" /> <span>{b}</span>
                    </li>
                  ))}
                </ul>
              </div>
        ...
        <section className="space-y-3">
          ...
          <div className="rounded-2xl bg-surface p-4 ring-1 ring-border">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Boost a listing to the top of Home, Search and Category pages for{" "}
              {prices?.featured?.duration_days ?? 7} days.
            </p>
            <div className="mt-2 flex items-center justify-between">
              <span className="text-lg font-bold text-primary">
                {formatINR(prices?.featured?.price ?? 49)}
              </span>
              <Link to="/my-ads" className="text-xs font-semibold text-primary">
                Pick a listing →
              </Link>
            </div>
          </div>
          ...
          <div className="rounded-2xl bg-surface p-4 ring-1 ring-border">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold">Bump listing</span>
            </div>
            <p className="mt-1 text-xs text-muted-foreground">
              Refresh a listing to the top of the feed. Cooldown{" "}
              {prices?.bump?.cooldown_hours ?? 24}h.
            </p>
            <div className="mt-2 flex items-center justify-between">
              <span className="text-lg font-bold text-primary">
                {formatINR(prices?.bump?.price ?? 19)}
              </span>
              <Link to="/my-ads" className="text-xs font-semibold text-primary">
                Pick a listing →
              </Link>
          ...
          <div className="rounded-2xl bg-surface p-4 ring-1 ring-border">
            ...
            <p className="mt-1 text-xs text-muted-foreground">
              Get a trust badge on your profile and listings after admin review.
            </p>
            <div className="mt-2 flex items-center justify-between">
              <span className="text-lg font-bold text-primary">
                {prices?.verification?.price ? formatINR(prices.verification.price) : "Free"}
              </span>
              <Link to="/verification" className="text-xs font-semibold text-primary">
                Apply →
              </Link>
            </div>
          </div>
        ...
        <div className="flex gap-2">
          <Link
            to="/subscription"
            className="flex-1 rounded-xl bg-surface p-3 text-center text-xs font-semibold ring-1 ring-border"
          >
            My subscription
          </Link>
          <Link
