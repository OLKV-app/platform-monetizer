    ...
    if (user) {
      try {
        ...
        if (!fsProfile?.full_name || !fsProfile?.island) {
          nav({ to: "/complete-profile", replace: true });
          return;
        }
      } catch (err) {
      ...
      <div className="mx-auto flex min-h-screen max-w-[430px] flex-col justify-center px-6 py-10">
        ...
        <Tabs defaultValue="email">
          ...
          <TabsContent value="email">
            <EmailAuth onSuccess={handlePostAuth} />
          </TabsContent>
          <TabsContent value="phone">
            <PhoneAuth onSuccess={handlePostAuth} />
          </TabsContent>
        </Tabs>
        ...
        <GoogleButton onSuccess={handlePostAuth} />
  ...
  return (
    <form onSubmit={submit} className="mt-4 space-y-3">
      ...
      <div className="flex items-center justify-between text-xs">
        {mode === "signin" ? (
          <>
            <button type="button" onClick={() => setMode("reset")} className="text-primary">
              {t("forgot_password")}
            </button>
...
function GoogleButton({ onSuccess }: { onSuccess?: () => void }) {
  ...
  async function go() {
    ...
    try {
      ...
      if (code === "auth/unauthorized-domain" || err?.message?.includes("unauthorized-domain")) {
        toast.error(
          `Domain (${window.location.hostname}) is not in Firebase Authorized Domains. Add it in Firebase Console > Authentication > Settings. Email & Password sign-in works without domain restriction!`,
          {
            duration: 8000,
          },
        );
  ...
  return (
    <Button variant="outline" onClick={go} disabled={busy} className="w-full gap-2">
      <svg width="16" height="16" viewBox="0 0 48 48">
        <path
          fill="#FFC107"
          d="M43.6 20.5H42V20H24v8h11.3C33.9 32.8 29.4 36 24 36c-6.6 0-12-5.4-12-12s5.4-12 12-12c3 0 5.8 1.1 7.9 3l5.7-5.7C34 6.1 29.2 4 24 4 12.9 4 4 12.9 4 24s8.9 20 20 20 20-8.9 20-20c0-1.2-.1-2.3-.4-3.5z"
        />
        <path
          fill="#FF3D00"
          d="M6.3 14.7l6.6 4.8C14.5 15.7 18.9 12.6 24 12.6c3 0 5.8 1.1 7.9 3l5.7-5.7C34 6.1 29.2 4 24 4 16.3 4 9.7 8.3 6.3 14.7z"
        />
        <path
          fill="#4CAF50"
          d="M24 44c5.1 0 9.7-1.9 13.2-5.1l-6.1-5c-2 1.4-4.5 2.2-7.1 2.2-5.4 0-9.9-3.2-11.3-8l-6.6 5.1C9.5 39.5 16.2 44 24 44z"
        />
        <path
          fill="#1976D2"
          d="M43.6 20.5H42V20H24v8h11.3c-.7 2-2 3.8-3.7 5.1l6.1 5c-.4.4 6.3-4.6 6.3-14.1 0-1.2-.1-2.3-.4-3.5z"
        />
      </svg>
